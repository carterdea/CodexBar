...
- Sparkle release key: this fork has none, and the upstream author's key has been removed from `.mac-release.env`. Never sign or publish with it. Releasing requires generating a fork-owned keypair first; until then `make release` is not a supported command here.
