...
struct SpendDashboardModel: Equatable, Sendable {
    ...
    /// Nil when no Claude source reported edit records for the window.
        ...
        preferredCurrencyCode: String = "auto") -> Self
    {
        ...
        return Self(
            requestedDays: days,
            groups: groups,
            tokenActivity: Self.tokenActivityPoints(
                inputs: inputs,
                now: now,
                calendar: calculationCalendar),
            codeWritten: Self.codeWritten(
                inputs: inputs,
                days: days,
                now: now,
                calendar: calculationCalendar))
    ...
    /// Sum the Claude edit counts that fall inside the requested window. Currency plays no part,
    /// so this is computed once across every source rather than per currency group.
    private static func codeWritten(
        inputs: [ProviderInput],
        days: Int,
        now: Date,
        calendar: Calendar) -> CostUsageEditCounts?
    {
        let bounds = Self.bounds(days: days, now: now, calendar: calendar)
        var total = CostUsageEditCounts.zero

        // Provider-specific by design: edit records exist only in Claude transcripts, so a sum that
        // reached past Claude would report a partial figure as a machine-wide one.
        for input in inputs where input.provider == .claude {
            for entry in input.snapshot.daily {
                guard let edits = entry.edits,
                      let day = Self.day(entry.date, provider: input.provider, displayCalendar: calendar),
                      bounds.contains(day)
                else { continue }
                total += edits
            }
        ...
        // Nothing edited is nothing to say. A window of pure research should not carry a "+0 / -0".
    ...
    /// Shared with `SpendDashboardModel+TokenActivity.swift`, which windows the same way.
